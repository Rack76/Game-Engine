...
namespace Internal {
	...
	template<typename _Event> requires std::same_as<_Event, typename std::decay<_Event>::type>
		...
		Callback(_Callable)->Callback<typename Callback_ctad_helper<decltype(&_Callable::operator())>::Type>;
		...
		private:
			/* Yes, std::list is slow, but it's the only container that never invalidates iterators, which is useful */
		...
		protected:
			...
			Event_emitter_base(Event_emitter_base const&) = default;
			Event_emitter_base& operator= (Event_emitter_base const&) = default;
...
protected:
	...
	Event_emitter(Event_emitter const&) = default;
	Event_emitter& operator= (Event_emitter const&) = default;
